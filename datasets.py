    ...
    def __getitem__(self, idx):
        ...
        try:
            if self.split in ["val_train", "val"]:
                qid = qid.split("_x")[0]
                human_att = cv2.imread("data/HumanAttention/vqahat_val/"+str(qid)+".png", cv2.IMREAD_GRAYSCALE)
            else:
                qid = qid.split("_")[0]
                #human_att = cv2.imread("/data/DataSets/VQA/HumanAttention/vqahat_train/"+str(qid)+"_1.png", cv2.IMREAD_GRAYSCALE)
                human_att = cv2.imread("data/HumanAttention/vqahat_train/"+str(qid)+"_1.png", cv2.IMREAD_GRAYSCALE)
