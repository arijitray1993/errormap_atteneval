...
class atten_error_helpfulness:
    ...
    def __init__(self):
        ...
        ques_data = json.load(open("/data/DataSets/VQA/OpenEnded_mscoco_val2014_questions.json")) # change the path to your VQA dataset location
...
class ErrorMaps:
    ...
    def __init__(self, device = None, w2v=None, data_val = None):
        ...
        if w2v==None:
            self.w2v = loadGloveModel("data/glove.6B.300d.txt")
        else:
            self.w2v = w2v
    ...
    def computeErrorMap(self, atten_weights, coco_id, question, answer_prob):
        ...
        model_out = self.attention_refine_model(atten_weights, im_feature, ques_feats, answer_prob)
        ...
        return err_cam, float(fail_pred.squeeze().detach().cpu())
    ...
    def compute_errmap_image(self, atten_weights, coco_id, question, answer_prob, errmap_im_file_name):
        ...
        att_map, fail_pred = self.computeErrorMap(atten_weights, coco_id, question, answer_prob) #att_map is the err map here
...
if __name__=="__main__":
    ...
    atten_weights = data_dictionary['attn'] # this could come from the live VQA model, but we are using a precomputed one here.
    ...
    answer_probs = data_dictionary['max_s'] # this could also come from live VQA model. This is a 1 x 3129 answer probabilities
    ...
    ############# tutorial of how to evaluate your attention/error maps for the VQA val IQ pairs ######################
    help_score_calc = atten_error_helpfulness()

    #example json file on how to format your attention and error map values.
    #assumes atten_map data as a dictionary of 
    # qid: ['answer': answer_text, 'atten': raw attention grid data, 'err': raw error grid data]
 
    atten_error_data = json.load(open("vqaqid2jatt.json"))

    eval_scores = help_score_calc.calc_helpfulness(atten_error_data)
    #eval_scores will be dict:
    # z_help_atten is atten helpfulness
    # z_help_err is error helpfulness
    # z_help_attenerr is joint attention-error helpfulness
    # rel_correct and rel_wrong are relevance values (correlation to human attention) for correct answer and wrong answer cases
